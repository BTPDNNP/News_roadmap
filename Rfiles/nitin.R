...
dataFile_strip <- "sensex.csv"
dataDir <- "/home/nitin/Desktop/"
file2 <- paste(dataDir,"/",dataFile_strip,sep="")

zz <- read.zoo(file2, sep = ",", header=TRUE,index.column=1,colClasses=c("character",rep("numeric",6)))
head(zz)

GSPC<- as.xts(zz)
GSPC<-GSPC[, c("Open", "High", "Low", "Close", "Volume")]
head(GSPC)
...
data.model <- specifyModel(TIND.roughfix ~ Delt.roughfix +
                          ...
                          CMO.roughfix + EMA.roughfix + EMV.roughfix+ 
...
rownames(imp)[which(imp > 4)]
data.model <- specifyModel(TIND.roughfix ~ Delt.roughfix +
                             ATR.roughfix + SMI.roughfix + ADX.roughfix +
                             + VOL.roughfix + SAR.roughfix + Mean.roughfix )
